...
const productDetailResponse: ProductDetailResponse = {
  ...
  "data": {
    "product_base": {
      ...
      "shop_base_id": "1__227642622",
      "platform_id": 1,
      "product_id_platform": 5382617241,
      "shop_id_platform": "227642622",
      "name": "Bộ Hub mở rộng Baseus Metal Gleam 2 Series 4K60Hz",
      "price": 526890,
      "price_before_discount": 579000,
      "category_base_id": "1__101987",
      "rating_detail": [
        1,
        0,
        0,
      ...
      "attributes": [
        ...
        },
        {
          ...
          "value": "Shenzhen Times Innovation Technology Co."
        },
        {
          "id": "101068",
          "name": "Địa chỉ tổ chức chịu trách nhiệm sản xuất",
          "value": "Room 6/F, Building 3, Winlead, Fada Road"
        }
      ],
      "variations": [
        {
          ...
          "options": [
            ...
            "10 in 1 Gleam2",
            "6 in 1 Gleam2",
            "8 in 1 4K60Hz USB3.1",
            "8 in 1 4K60Hz USB3.0",
            "6 in 1 4K60Hz",
            "4.1",
            "4.2",
            "4.3",
      ...
      "categories": [
        ...
        {
          "category_base_id": "1__101987",
          "slug": "bo-chia-cong-usb-doc-the-nho",
          "name": "Bộ chia cổng USB & Đọc thẻ nhớ",
          "url_thumbnail": "https://i.imgur.com/LZZhAkE.png",
          "cat_level": 2,
          "parent_category_id": "1__101940",
          "relative_path": null
        }
      ],
      "price_insight": {
...
const productData = productDetailResponse.data.product_base

const page = ref(0)
const currentImage = computed(() => {
  return productData.url_images[page.value]
})
</script>

<template>
  <div class="max-w-7xl mx-auto">
    <div class="my-16 w-full flex items-center lg:items-start flex-col lg:flex-row mb-4 lg:mb-6 px-2 sm:px-0">
      <div id="carousel" class="w-full lg:w-1/2 flex-shrink-0 mr-4 lg:p-6 lg:border rounded-xl">
        <div>
          <div class="p-3">
            <img :src="currentImage" :alt="productData.name" class="w-[550px]">
          </div>
          <div class="w-full overflow-x-auto whitespace-nowrap flex gap-2">
            <div v-for="img, i in productData.url_images" :key="i" class="border p-0.5" @mouseover="page = i">
              <img :src="img" :alt="productData.name" class="w-24 h-24">
            </div>
          </div>
        </div>
      </div>
      <div id="info">
        ...
        <p
          class="lg:w-44 flex items-center text-orange-500 font-medium cursor-pointer lg:hover:text-white lg:hover:bg-orange-500 lg:border lg:border-orange-500 rounded-2xl lg:px-2 py-1 my-2 lg:my-4">
        ...
        <div class="flex lg:flex-col justify-between flex-col">
          <div
            ...
            <div>
              <p class="text-red-600 text-2xl font-bold">{{ productData.price }} ₫</p>
              <p class="text-sm text-gray-500 line-through">{{ productData.price_before_discount }} ₫</p>
            </div>
            <div class="flex items-center gap-2 bg-green-400 rounded-lg p-1 text-green-900">
              <UIcon :name="productData.price_insight.delta_price < 0 ? 'i-ph-trend-down' : 'i-ph-trend-up'"
                class="w-4 h-4" />
              <span class="text-xs font-medium">{{ productData.price_insight.delta_price }} ₫</span>
            </div>
            <div class="text-white px-4 py-2 bg-orange-500 hover:font-semibold flex items-center gap-2 rounded">
        ...
        <div class="flex items-center text-xs lg:text-sm">
          <div class="px-2 text-amber-400 flex items-center border-r-2">
            ...
            <UIcon name="i-ph-star-fill" class="w-5 h-5" />
            <UIcon name="i-ph-star-fill" class="w-5 h-5" />
            <UIcon name="i-ph-star-fill" class="w-5 h-5" />
            <UIcon name="i-ph-star-fill" class="w-5 h-5" />
            <UIcon name="i-ph-star-half-fill" class="w-5 h-5" />
          </div>
    ...
    <div>
      <div class="sticky top-0 w-full bg-white">
        <ul class="flex gap-1">
          <li class="px-2 py-3 flex items-center gap-2 border-b-2 border-orange-500">
            <a href="#price-compare">
              <UIcon name="i-fluent-mdl2-compare" class="w-5 h-5" /> <span class="font-medium">So sánh giá</span>
            </a>
          </li>
          <li class="px-2 py-3 flex items-center gap-2 border-b-2">
            <a href="#price-history">
              <UIcon name="i-ph-chart-line" class="w-5 h-5" /> <span class="font-medium">Lịch sử giá</span>
            </a>
          </li>
        </ul>
      </div>

      <div id="price-compare" class="mt-2 scroll-mt-20">
        <div class="lg:text-lg font-semibold">So sánh giá</div>
        <ProductComparePrice />
      </div>
      <div id="price-history" class="mt-4 scroll-mt-16">
        <div class="lg:text-lg font-semibold">Lịch sử giá</div>

      </div>
      <div id="product-description" class="mt-4 scroll-mt-16">
        <div class="lg:text-lg font-semibold mb-4">Mô tả sản phẩm</div>
        <div v-html="productData.description.replace(/(?:\r\n|\r|\n)/g, '<br>')"></div>
      </div>
    </div>
  </div>
</template>
